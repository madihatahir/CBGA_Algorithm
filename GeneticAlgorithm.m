...
function [BestChrom, population]  = GeneticAlgorithm (feat,label, M , N, MaxGen , Pc, Pm , Er , obj, visuailzation)
...
for i = 1 : M
    population.Chromosomes(i).fitness = obj( feat, label, population.Chromosomes(i).Gene(:) );
end
...
for g = 2 : MaxGen
    ...
    for i = 1 : M
        population.Chromosomes(i).fitness = Sphere(feat,label, population.Chromosomes(i).Gene(:) );
    end
    ...
    for i = 1 : M
        newPopulation.Chromosomes(i).fitness = obj( feat,label,newPopulation.Chromosomes(i).Gene(:) );
    end
    % Elitism
